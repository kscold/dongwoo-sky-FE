 ...
 * 관리자용 홈 설정 단일 조회
 */
export const getAdminHomeSetting = async (
  id: string
...
export const updateAdminHomeSettings = async (
  ...
  try {
    ...
    console.log(
      `[updateAdminHomeSettings] 홈 설정 업데이트 완료:`,
      response.data
    )
    return response.data
  } catch (error) {
    console.error(
      `[updateAdminHomeSettings] 관리자 홈 설정 업데이트 실패:`,
      error
    )
    throw error
  }
...
export const createAdminHomeSettings = async (
  ...
  try {
    const response = await apiClient.post<HomeSettings>("/admin/home", settings)
