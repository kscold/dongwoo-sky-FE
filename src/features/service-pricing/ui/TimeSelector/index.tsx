"use client"

import React from "react"
import { Equipment } from "../../../../types/equipment"
import * as styles from "../../styles"

interface TimeSelectorProps {
  selectedEquipment: Equipment | null
  workingHours: number
  onWorkingHoursChange: (hours: number) => void
  settings: any
  timeSelectionLabel?: string
  hourUnit?: string
  onContactClick: () => void
}

export function TimeSelector({
  selectedEquipment,
  workingHours,
  onWorkingHoursChange,
  settings,
  timeSelectionLabel,
  hourUnit,
  onContactClick,
}: TimeSelectorProps) {
  if (!selectedEquipment) {
    return (
      <div className={styles.timeSelector}>
        <div className={styles.timeDisplay}>
          <span className={styles.timeLabel}>
            {timeSelectionLabel || "작업 시간 선택"}
          </span>
          <span className={styles.timeValue}>장비를 먼저 선택해주세요</span>
        </div>
        <button className={styles.ctaButton} onClick={onContactClick}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" style={{marginRight: "8px"}}>
            <path d="M22 16.92V19.92C22.0011 20.1985 21.9441 20.4742 21.8325 20.7293C21.7209 20.9845 21.5573 21.2136 21.3521 21.4019C21.1468 21.5901 20.9046 21.7335 20.6407 21.8227C20.3769 21.912 20.0974 21.9452 19.82 21.92C16.7428 21.5856 13.787 20.5341 11.19 18.85C8.77382 17.3147 6.72533 15.2662 5.18999 12.85C3.49997 10.2412 2.44824 7.27099 2.11999 4.18C2.09501 3.90347 2.12787 3.62476 2.21649 3.36162C2.30512 3.09849 2.44756 2.85669 2.63476 2.65162C2.82196 2.44655 3.0498 2.28271 3.30379 2.17052C3.55777 2.05833 3.83233 2.00026 4.10999 2H7.10999C7.5953 1.99522 8.06579 2.16708 8.43376 2.48353C8.80173 2.79999 9.04207 3.23945 9.10999 3.72C9.23662 4.68007 9.47144 5.62273 9.80999 6.53C9.94454 6.88792 9.97366 7.27691 9.89391 7.65088C9.81415 8.02485 9.62886 8.36811 9.35999 8.64L8.08999 9.91C9.51355 12.4135 11.5864 14.4864 14.09 15.91L15.36 14.64C15.6319 14.3711 15.9751 14.1858 16.3491 14.1061C16.7231 14.0263 17.1121 14.0555 17.47 14.19C18.3773 14.5286 19.3199 14.7634 20.28 14.89C20.7658 14.9585 21.2094 15.2032 21.5265 15.5775C21.8437 15.9518 22.0122 16.4296 22 16.92Z" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          {settings.ctaButtonText || "직접 문의하고 할인받기"}
        </button>
      </div>
    )
  }

  const percentage =
    ((workingHours - selectedEquipment.minHours) /
      (selectedEquipment.maxHours - selectedEquipment.minHours)) *
    100

  return (
    <div className={styles.timeSelector}>
      <div className={styles.timeDisplay}>
        <span className={styles.timeLabel}>
          {timeSelectionLabel || "작업 시간 선택"}
        </span>
        <span className={styles.timeValue}>
          {workingHours} {hourUnit || "시간"}
        </span>
      </div>
      <div className={styles.timeSlider}>
        <input
          type="range"
          min={selectedEquipment.minHours}
          max={selectedEquipment.maxHours}
          value={workingHours}
          onChange={(e) => onWorkingHoursChange(Number(e.target.value))}
          className={styles.slider}
          style={{ "--value": `${percentage}%` } as React.CSSProperties}
        />
        <div className={styles.sliderLabels}>
          <span>
            {selectedEquipment.minHours} {hourUnit || "시간"}
          </span>
          <span>
            {selectedEquipment.maxHours} {hourUnit || "시간"}
          </span>
        </div>
      </div>
      <button className={styles.ctaButton} onClick={onContactClick}>
        📞 {settings.ctaButtonText || "직접 문의하고 할인받기"}
      </button>
    </div>
  )
}